...
begin
	...
	using GeometricAlgebra, Mooncake, Plots, Test
end

# ╔═╡ 3a4beb90-7ef4-435c-a760-e814fda50b2f
md"""
This notebook shows a simple example of finding a 3D rotor by minimising a loss function by gradient descent using automatic differentiation.
"""
...
Consider a set of ``N`` points ``r_i \in \mathbb{R}^3`` which are transformed as ``R^\star r_i \tilde{R^\star}`` by an unknown rotor ``R^\star``.

Suppose we measure the locations of each point's "shadow", that is, we measure the projections ``p_i`` of each point in the ``xy``-plane, given by the bivector ``\pi = e_1 e_2``.

*Problem:* Find the rotor ``R^\star`` given the points ``r_i`` and their shadows ``p_i = (R^\star r_i \tilde{R^\star}) \cdot \pi \, \pi^{-1}``.
"""
...
r = randn(Multivector{3,1}, N); # array of random points

# ╔═╡ 7933ded8-5591-4907-ad81-39d4ac0e2f7b
R_star = exp(10*randn(Multivector{3,2})) # target rotor

# ╔═╡ c16073d8-d9f2-445e-8b55-084ed216b572
plane = Multivector{3,2}([1,0,0]); # bivector representing xy-plane

# ╔═╡ 4f511640-9817-47d2-81a7-6194d101c1aa
md"We compute ``p_i`` to simulate measurements of the point shadows:"
...
# ╔═╡ 60480028-b53f-4630-87c3-6d2a3627aff0
md"### Imports"
