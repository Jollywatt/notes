...
We can combine incident/orthogonal components in various ways to obtain various reflections:
$
rej(u, A) - proj(u, A) &= A^star u A^(-1)
	& #h(1cm) &#[reflect $u$ #underline[within] $A$] \
proj(u, A) - rej(u, A) &= -A^star u A^(-1)
	& &#[reflect $u$ #underline[across] $A$] \
rej(A, u) - proj(A, u) &= u A^star u^(-1)
	& &#[reflect $A$ #underline[within] $u$] \
proj(A, u) - rej(A, u) &= -u A^star u^(-1)
	& &#[reflect $A$ #underline[across] $u$] \
$

To show these, use the definitinos from @multivector-proj-rej and the lemma in @reversion-sign.
For example:
$
	rej(u, A) - proj(u, A)
	&= (u wedge A - u lcont A)A^(-1) \
	&= (A^star wedge u - rev(rev(A) rcont u))A^(-1) \
	&= (A^star wedge u - revsign(k)revsign(k - 1) A rcont u)A^(-1) \
	&= (A^star wedge u + A^star rcont u)A^(-1) \
	&= A^star u A^(-1) \
$