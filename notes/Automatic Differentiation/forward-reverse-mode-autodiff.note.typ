...
#let dwrt(var) = $diff/(diff italic(var))$ // derivative with respect to
#let terminal = $star$
...
#let show-program(program) = {
	...
	let lines = get-steps(program)
		...
		.map( ((step, deps)) => $step && ~~> #fmtnone(deps.map(math.italic).join($, $))$)
		.join($\ $)
	$ lines $
}
...
#let show-inline-program(program) = {
	let inputs = get-inputs(program).map(italic).join($, $)
	...
	$
	#vec(..outputs.map(var => $italic(var) (inputs)$))  := #vec(..outputs.map(substitute))
	$
}
...
#let get-forward(program) = {
	program.pairs().map(((name, (_, dfn))) => {
		let rhs = dfn.pairs()
			.map(((var, der)) => $der dif italic(var)$)
			.join($op(+)$)
		$dif italic(name) &:= fmtnone(rhs)$
	})
}
...
#let get-reverse(program, inputs: true) = {
	...
	steps.map(((var, rhs)) => $diff italic(var) &:= fmtnone(rhs)$)
}
...
#let lastvar = math.italic(program.keys().last())
...
#let show-forward-program(program) = {
	let lines = program.pairs().map(((name, (fn, dfn))) => {
		let rhs = dfn.pairs()
			.map(((var, der)) => $der dif italic(var)$).join($op(+)$)
		if fn == none { return }
		(
			$italic(name) &:= fn$,
			$dif italic(name) &:= fmtnone(rhs)$,
	...
	let outputs = get-outputs(program)
	let args = $(#inputs.map(math.italic).join($, $) semi #inputs.map(var => $dif italic(var)$).join($, $))$
	let returns = $(#outputs.map(math.italic).join($, $) semi #outputs.map(var => $dif italic(var)$).join($, $))$
	show: program-box
	set align(left)
	braceblock(
...
This function computes both #get-outputs(program).map(var => $var$).join[, ] and the differential #get-outputs(program).map(var => $dif italic(var)$).join[, ] in one pass.
...
#let func(inputs, outputs, steps, prefix: none) = {
	...
	align(left)[
		$prefix (#inputs.map(math.italic).join($, $)) |-> {$ \
		#pad(left: 2em, $ #steps.join($\ $) $)
		$} -> (#outputs.map(math.italic).join($, $))$
	]
}
...
forward(func(
	...
	#(get-inputs(program) + get-inputs(program).map(var => $dif italic(var)$)),
	#(get-outputs(program) + get-outputs(program).map(var => $dif italic(var)$)),
	#get-body(program).map( ((var, (fn, dfn))) => {
		$
		(italic(var), dif italic(var)) &:=
		forward(fn)(#dfn.keys().join($, $), #dfn.keys().map(var => $dif italic(italic(var))$).join($, $)) = 
		(fn, #dfn.pairs().map( ((var, der)) => $ der dif italic(var) $).join($+$))
		$
	}),
)
$
...
#let show-reverse-program(program) = {
	...
	let returns = $(#outputs.join($, $) semi #inputs.map(var => $diff italic(var)$).join($, $))$
	show: program-box
	set align(left)
	braceblock(
		head: $"function" args space $,
		..get-steps(program, inputs: false),
		braceblock(
			head: $JJ &:= (#outputs.map(var => $diff italic(var)$).join($, $)) |-> $,
			..get-reverse(program, inputs: false),
			tail: $-> (#inputs.map(var => $diff italic(var)$).join($, $))$,
		),
	...
	diagram(
		...
		for (l, layer) in layers.enumerate() {
			for (v, var) in layer.enumerate() {
				...
				node((l, v - layer.len()/2), $ lhs italic(outputs.at(var)) $, name: var)
...
#flow-diagram(
	...
	program.pairs().map( ((var, (fn, dfn))) => (var, $(italic(var), dif italic(var))$)).to-dict(),
)
