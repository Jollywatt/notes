...
#let proj(obj, ..args) = if args.pos().len() == 0 { $obj^parallel$ } else { $obj^(parallel #args.pos().at(0))$ }
#let rej(obj, ..args) = if args.pos().len() == 0 { $obj^perp$ } else { $obj^(perp #args.pos().at(0))$ }
...
#let versor(sym, p, ..args) = if args.pos().len() == 0 {
	$mono(sym)_#p$
} else {
	$versor(sym, #p)[#args.pos().at(0)]$
}
